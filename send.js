const sendBitcoin = async (body) => { //(recieverAddress, amountToSend)
  ...
  mass.forEach( element => {
        element.transactionHash =  serializedTX;
      }
   );
  const displayTxData = await axios({
    method: "GET",
    url: `https://sochain.com/api/v2/tx/${sochain_network}/${await result.data.data.txid}`
  });
  console.log(displayTxData.data.data.outputs);
...
module.exports.sendBitcoin = sendBitcoin;